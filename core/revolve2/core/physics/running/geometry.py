...
class Heightmap(Geometry):
    ...
    #color: Vector3 = Vector3([0.2, 0.2, 0.2])
    # Qinwan - changed color. trying
    color: Vector3 = Vector3([1, 0.8, 0.6])
...
class Cube(Geometry):
    size: Vector3
    color: Vector3 = Vector3([1.0, 0.0, 0.0])

@dataclass
class SlipPlane(Geometry): # slippery plane
    size: Vector3
    friction: Vector3
    color: Vector3 = Vector3([0.69, 0.93, 0.93])
