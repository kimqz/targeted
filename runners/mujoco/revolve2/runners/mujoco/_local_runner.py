...
#Qinwan # this is for random target
import random
...
class LocalRunner(Runner):
    ...
    def _run_environment(
        ...
        if not headless or record_settings is not None:
            ...
            viewer.cam.lookat[0] = model.stat.center[0]
            viewer.cam.lookat[1] = model.stat.center[1]
            viewer.cam.lookat[2] = model.stat.center[2]
            #"
        ...
        results.environment_states.append(
            ...
            # Qinwan
            EnvironmentState(0.0, cls._get_actor_states(env_descr, data, model), env_descr.target_position)
            # "
        )
        ...
        while (time := data.time) < simulation_time:
            ...
            if time >= last_control_time + control_step:
                ...
                # Qinwan needed to comment out above line to add below "
                actorState = cls._get_actor_states(env_descr, data, model)
                target_pos = env_descr.target_position
                env_descr.controller.control(control_step, control_user, 'open', np.array([0, 0, 0, 0, 0, 0, 0, 0]),
                                             actorState, target_pos)
                # end"
        ...
        for geo in env_descr.static_geometries:
            if isinstance(geo, geometry.Plane):
                ...
                env_mjcf.asset.add('texture', name="grid", type="2d", builtin="checker",
                              width="512", height="512", rgb1=".1 .2 .3", rgb2=".7 .8 .9") #rgb1=".1 .2 .3", rgb2=".2 .3 .4")
            ...
            # Qinwan "
            elif isinstance(geo, geometry.Cube):
                if geo.position == Vector3():
                    # Generate a random position with the same distance from center
                    #theta = 2 * math.pi * random.random()
                    #pos_x = 2 * math.cos(theta)
                    #pos_y = 2 * math.sin(theta)
                    pos_x = 0
                    pos_y = 2
                else:
                    # Use the position set in the cube.
                    pos_x = geo.position.x
                    pos_y = geo.position.y

                pos_x = 0
                pos_y = 2

                # new. adding positon to
                env_descr.target_position = Vector3([pos_x, pos_y, geo.size.z / 2.0])

                env_mjcf.worldbody.add('geom', name="cube",
                                       size=[geo.size.x / 2.0, geo.size.y / 2.0, geo.size.z / 2.0],
                                       pos=[pos_x, pos_y, geo.size.z / 2.0],
                                       type="sphere", rgba=[geo.color.x, geo.color.y, geo.color.z, 1.0])

            elif isinstance(geo, geometry.SlipPlane):
                rgb1_str = f"{geo.color.x} {geo.color.y} {geo.color.z}"
                env_mjcf.asset.add('texture', name="grid", type="2d", builtin="checker",
                ...
                env_mjcf.asset.add('material', name="grid", texture="grid",
                                   texrepeat="1 1", texuniform="true", reflectance="0")
                env_mjcf.worldbody.add('geom', name="slip", type="plane",
                                       size=[geo.size.x / 2.0, geo.size.y / 2.0, geo.size.z / 2.0],
                                       material="grid",
                                       friction=[geo.friction.x, geo.friction.y, geo.friction.z])
            # end "
